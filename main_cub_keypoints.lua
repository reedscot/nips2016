...
Generic training script for CUB GAWWN keypoints,txt -> image.
...
if opt.init_g == '' then
  ...
  netG = nn.Sequential()
    ...
    conc:add(conv)
    netG:add(conc)
  netG:add(nn.CAddTable()):add(nn.ReLU(true))
    -- state size: (ngf*4) x 16 x 16
    :add(SpatialFullConvolution(ngf * 4, ngf * 2, 4, 4, 2, 2, 1, 1))
    :add(SpatialBatchNormalization(ngf * 2)):add(nn.ReLU(true))
...
local sample = function()
  ...
  real_img, real_txt, real_loc, _ = data:getBatch()
  data_tm:stop()
...
for epoch = 1, opt.niter do
  ...
  for i = 1, math.min(data:size(), opt.ntrain), opt.batchSize do
    ...
    if ((i-1) / opt.batchSize) % opt.print_every == 0 then
      ...
      disp.image(real_img:narrow(1,1,math.min(4,opt.batchSize)), {win=opt.display_id * 3, title=opt.name})
    end
  end
